...
/** Writes the fingerprints from memory to the file system. If a file path is not provided it will use
the path specified when the object was created.
 * @method
 * @param {string} [filename] - destination path to fingerprints file.
 * @throws {Error}
 */
User.prototype.saveFingerprintsToFS = function (filename) {
	this.state.writeFingerprintsSync(expandHomeDir(filename) || this.fingerprints);
};

/** Writes the private keys from memory to the file system. Normally you do not have to call this method, when new
	keys are generated or imported they will be automatically written to the keys file specified when User object was created.
	You can call this method to save the keys to a different destination. If a file path is not provided it will use
	the path specified when the object was created.
	* @method
	* @param {string} [filename] - destination path to private keys file.
	* @throws {Error}
	*/
User.prototype.saveKeysToFS = function (filename) {
	this.state.writeKeysSync(expandHomeDir(filename) || this.keys);
};

/** Writes the instance tags from memory to the file system. If a file path is not provided it will use
the path specified when the object was created.
 * @method
 * @param {string} [filename] - destination path to instags file.
 * @throws {Error}
 */
User.prototype.saveInstagsToFS = function (filename) {
	this.state.writeInstagsSync(expandHomeDir(filename) || this.instags);
};
...
/** Writes the fingerprints from memory to the file system. This method can be used optionally to save new fingerprints
	...
	* @throws {Error}
	*/
User.prototype.writeFingerprints = User.prototype.saveFingerprintsToFS;
