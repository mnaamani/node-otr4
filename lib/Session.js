...
function Session(user, account, contact, parameters) {
	...
	events.EventEmitter.call(session);
	...
	session.ops = new libotr.MessageAppOps(
		function (o) {
			...
			case "received_symkey":
				 ...
				 */
				session.emit(o.EVENT, o.use, (new Uint8Array(o.usedata)).buffer, (new Uint8Array(o.key)).buffer);
				return;
			...
			case "update_context_list":
				/** "update_context_list" Session event is fired when session state changes,
				 * to inform application to update UI/visual state of a session.
	...
	session.message_poll_interval = setInterval(function () {
...
Session.prototype.send = function (message, instag) {
	...
	nextTick(function () {
		...
		var msgout = session.ops.messageSending(session.user.state, session.context.accountname(),
			session.context
			.protocol(),
			session.context.username(), buff.toString(), instag, session.context);
		if (msgout) {
...
Session.prototype.recv = function (message) {
	...
	nextTick(function () {
		...
		var msg = session.ops.messageReceiving(session.user.state, session.context.accountname(),
			session.context
			.protocol(),
			session.context.username(), buff.toString(), session.context);
		if (msg) {
...
/** Returns the extra symmetric key and informs contact that we want to use it. The key is returned as the first
...
Session.prototype.extraSymKey = function (use, usedata, callback) {
	...
	var session = this;
	nextTick(function () {
		var buf = session.ops.extraSymKey(session.user.state, session.context, use, usedata); //returns a Buffer
		var key = new Uint8Array(buf).buffer;
		callback(key);
	});
};
